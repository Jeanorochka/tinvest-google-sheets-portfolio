/**
 * Tinkoff Invest → Google Sheets (лоты + ₽ + агрегация + средняя цена/шт)
 * Язык: Google Apps Script (JavaScript, V8).
 *
 * Листы:
 * - Positions — сырые позиции
 * - Positions_Aggregated — агрегировано по имени/тикеру/FIGI (см. AGGREGATE_BY)
 *
 * Перед запуском: Project Settings → Script properties → TINKOFF_TOKEN = t.xxxxx...
 */
...
const BASES = [
  ...
  'https://invest-public-api.tinkoff.ru/rest', // фолбэк
];
...
function getInstrumentByFigi_(figi) {
  ...
  const out = data ? {
    ...
    instrumentType: data.instrumentType || data.type || '',
    currency: (data.currency || data.nominalCurrency || '').toUpperCase(),
  } : null;
...
function writeTable_(sheetName, rows) {
  ...
  if (rows.length > 1) {
    ...
    const idx = Object.fromEntries(header.map((k, i) => [k, i + 1])); // 1-based для Range
...
function syncTinkoffPositions() {
  const header = [
    ...
    'name',
    'lot',
    'quantity_lots',               // кол-во лотов (qty_pcs / lot)
  ...
  for (const acc of accounts) {
    ...
    for (const p of positions) {
      ...
      const qtyPcs = qToNumber_(p.quantity); // штуки

      // Текущая цена за 1 шт (₽) и средняя покупочная цена за 1 шт (₽)
  ...
  const key = AGGREGATE_BY;
  if (['name','ticker','figi'].includes(key)) {
    const agg = aggregateByKey_(raw, key);
    writeTable_(AGG_SHEET, agg);
  }
}
...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('Tinkoff')
    .addItem('Синхронизировать (лоты + ₽ + аггр.)', 'syncTinkoffPositions')
    .addToUi();
}

// (опционально) Самотест
function testInfo() {
